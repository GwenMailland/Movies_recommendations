...
# Set up SparkConf
conf = SparkConf().setAppName("gwen").setMaster("local[4]")
...
# Load and parse the data ratings
...
movies_data=sc.textFile("ml-10M100K/movies.dat")
...
#mostRatedMovies
movie_ratings_count = ratings.map(lambda r: (r[1], 1)).reduceByKey(lambda x, y: x + y)
top200_movie=movie_ratings_count.sortBy(lambda r:(r[1]),ascending=False).take(200)

mostRatedMovies=[]
for movie_id, count in top200_movie:
    movie_title = movies.lookup(movie_id)[0]
    mostRatedMovies.append((movie_id,movie_title))
...
for recommendation in top_recommendations:
    ...
    print(f"Movie ID: {movie_id}, Title: {movie_title}, Predicted Rating: {recommendation.rating}")
